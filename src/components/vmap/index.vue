...
const initCesium = () => {
  ...
  addDynamicAreasAndLabels(viewer,testData);
...
const testData = [
  {
    "id": "test_area_init",           // 区域ID（可选，不填会自动生成）
    "name": "海尔机器人",              // 标签文字
    "height": 10,                     // 区域高度（米）
    "color": "#FFA500",               // 区域颜色 (可选，默认橙色)
    "points": [                       // 多边形坐标 [lon, lat, lon, lat...]
      116.519451, 39.780289,
      116.519891, 39.780521,
      116.520002, 39.780399,
      116.519560, 39.780166
    ],
    "labelPosition": {                // 标签位置
      "lon": 116.519708,
      "lat": 39.780339,
  ...
  // --- 添加第二个区域演示 ---
  {
    ...
    "color": "#00FF00",               // 绿色区域
    "points": [
      116.520500, 39.780289,
      116.520900, 39.780521,
      116.521000, 39.780399,
      116.520600, 39.780166
    ],
    "labelPosition": {
      "lon": 116.520750,
      "lat": 39.780339,
 ...
 */
function addDynamicAreasAndLabels(viewer, dataList) {
  ...
  dataList.forEach((item, index) => {
    ...
    // -----------------------------------------------------------
    // 如果没有提供ID，生成一个唯一ID
    const areaId = item.id || `dynamic_area_${index}`;
    
    // 多边形坐标数组 (平面数组: [lon, lat, lon, lat...])
    const points = item.points;
    
    // 标签位置 {lon, lat, height}
    const labelPos = item.labelPosition;

    // 样式配置（如果JSON中没有提供，则使用默认值）
    const height = item.height !== undefined ? item.height : 10;
    const labelText = item.name || "未命名区域";
    
    // 处理颜色：支持传入CSS颜色字符串，如果没有则默认为橙色
    const polygonColor = item.color 
      ? Cesium.Color.fromCssColorString(item.color).withAlpha(item.alpha || 0.1) 
      : Cesium.Color.ORANGE.withAlpha(0.7);

    const bgColor = item.backgroundColor 
      ? Cesium.Color.fromCssColorString(item.backgroundColor).withAlpha(0.8)
      : Cesium.Color.fromCssColorString("#333333").withAlpha(0.8);
    ...
    viewer.entities.add({
      ...
      polygon: {
        ...
        outlineWidth: 4,
        height: height,
        classificationType: Cesium.ClassificationType.BOTH // 贴地且贴模型
      }
    });
