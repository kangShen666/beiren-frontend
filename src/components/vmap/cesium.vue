...
const _bingMap1 = new Cesium.UrlTemplateImageryProvider({
...
const imgList = [
  // 西1
  {
    ...
    startLnt1: 116.518955,
    startLat1: 39.778661,
    startLnt2: 116.51901257365573,
    startLat2: 39.77859518594504,
    startLnt3: 116.518946,
    startLat3: 39.778561,
    startLnt4: 116.5188862646933,
    startLat4: 39.77862666778118,
    rote: 60,
    strote: 60,
  },
  {
    ...
    startLnt1: 116.519012,
    startLat1: 39.778689,
    startLnt2: 116.519069,
    startLat2: 39.778621,
    startLnt3: 116.51901257365573,
    startLat3: 39.77859518594504,
    startLnt4: 116.518955,
    startLat4: 39.778661,
    rote: 60,
    strote: 60,
  },
  ...
  // 南1
  {
    id: 'i5',
    imgUrl: './public/img/111.png',
    startLnt1: 116.52108742196226,
    startLat1: 39.77795701842022,
    startLnt2: 116.52119330370945,
    startLat2: 39.77800761702931,
    startLnt3: 116.52123692947193,
    startLat3: 39.77795460211267,
    startLnt4: 116.52113242856868,
    startLat4: 39.77790391441666,
    rote: -30,
    strote: -30,
  },
  {
    id: 'i6',
    imgUrl: './public/img/111.png',
    startLnt1: 116.5211930190762,
    startLat1: 39.778007407145786,
    startLnt2: 116.52128909150268,
    startLat2: 39.77805383351373,
    startLnt3: 116.52132734335534,
    startLat3: 39.77799829594331,
    startLnt4: 116.52123543881095,
    startLat4: 39.77795421858285,
    rote: -30,
    strote: -30,
  },
  {
    id: 'i7',
    imgUrl: './public/img/222.png',
    startLnt1: 116.52113494245344,
    startLat1: 39.777905237859414,
    startLnt2: 116.5212318554277,
    startLat2: 39.77795242388284,
    startLnt3: 116.52128567658136,
    startLat3: 39.77787734103089,
    startLnt4: 116.5211926979105,
    startLat4: 39.77783323437208,
    rote: -30,
    strote: -30,
  },
  {
    id: 'i8',
    imgUrl: './public/img/222.png',
    startLnt1: 116.52132739384494,
    startLat1: 39.77799836124578,
    startLnt2: 116.5212318554277,
    startLat2: 39.77795242388284,
    startLnt3: 116.52128580487773,
    startLat3: 39.77787706301688,
    startLnt4: 116.52138086364472,
    startLat4: 39.777921619197365,
    rote: -30,
    strote: -210,
  },
  // 南二
  ...
  // 北门
  {
    id: 'i13',
    imgUrl: './public/img/222.png',
    startLnt1: 116.51846192833295,
    startLat1: 39.78119697340168,
    startLnt2: 116.51826122197473,
    startLat2: 39.78109403846232,
    startLnt3: 116.51817369927403,
    startLat3: 39.78119622766197,
    startLnt4: 116.51837596299556,
    startLat4: 39.78129844864408,
    rote: 150,
    strote: 150,
    height: 3,
  },
  {
    id: 'i14',
    imgUrl: './public/img/111.png',
    startLnt1: 116.5185484460368,
    startLat1: 39.78108838706317,
    startLnt2: 116.5183416168613,
    startLat2: 39.78098899341698,
    startLnt3: 116.51826066966835,
    startLat3: 39.781094445873435,
    startLnt4: 116.51846097247095,
    startLat4: 39.78119788868953,
    rote: 150,
    strote: 150,
    height: 3,
  },
...
const gaodidianliandong = (hotspotList: HotspotEntity[]) => {
  ...
  hotspotList.forEach((hotspot) => {
    const { id, name, duankouhao, coords } = hotspot
    try {
      // 创建多边形坐标点（添加高度值）
    ...
    async (event: { position: Cesium.Cartesian2 }) => {
      ...
      if (Cesium.defined(pickedObject)) {
        ...
        if (entity && entity.properties) {
          ...
          //   return;
          // }
          const hotspotData = entity?.properties?.hotspot?._value
...
const trailer1Ref = ref()
const trailer2Ref = ref()
const trailer3Ref = ref()
const trailer4Ref = ref()
const trailer5Ref = ref()
const trailer6Ref = ref()
const trailer7Ref = ref()
const trailer9Ref = ref()
const trailer31Ref = ref()
...
const trailer36Ref = ref()
const trailer37Ref = ref()
const trailer38Ref = ref()

// ✅ 每个视频实体对应的 stop 句柄，key = 实体 id（q1/q2/...）
const videoStopHandles: Record<string, () => void> = {}
...
let QuanJing = function (e, idArray) {
  ...
  const videoEntities = [
    // 第二版视频点位
    {
      ...
      x: -2191837.840988996,
      y: 4391846.580846679,
      z: 4059218.7812181143,
      pitch: -0.27361528074335073,
      heading: 4.133470092831442,
    },
    {
      id: "q6",
      hierarchy: [
      ...
      visible: !e,
      x: -2191776.6745118657,
      y: 4391829.610455442,
      z: 4059267.6261500968,
      pitch: -0.27361528788557665,
      heading: 4.133470095898148,
    },
    ...
    },
    {
      ...
      visible: !e,
      x: -2191803.2847895636,
      y: 4391837.034217936,
      z: 4059245.2259718766,
      pitch: -0.27361528479024466,
      heading: 4.13347009456906,
    },
    {
      id: "q7",
      hierarchy: [
    ...
    },
    {
      ...
      visible: !e,
      x: -2191789.9405385195,
      y: 4391833.593338022,
      z: 4059256.154013882,
      pitch: -0.27361528630033405,
      heading: 4.13347009521747,
    },
    {
      id: "q1",
      hierarchy: [
      ...
      x: -2191857.762064247,
      y: 4391851.216336267,
      z: 4059207.8195358147,
      pitch: -0.273615278180126,
      heading: 4.133470091730841,
    },
    {
      id: "q3",
      hierarchy: [
    ...
    // C
    {
      ...
      visible: !e,
      x: -2191733.8457237356,
      y: 4391820.719662465,
      z: 4059327.315948713,
      pitch: -0.8485225161402923,
      heading: 4.1272352075236025,
    },
    // c中
    {
      id: "q42",
      ...
      x: -2191714.377069298,
      y: 4391812.384958129,
      z: 4059346.8448545695,
      pitch: -0.848522518864169,
      heading: 4.127235212189515,
    },
    // c北
    {
      id: "q43",
      ...
      x: -2191693.340947181,
      y: 4391802.476533532,
      z: 4059368.9224454653,
      pitch: -0.8485225219434982,
      heading: 4.127235217464099,
    },
  ]
  ...
  const createVideoEntitySafe = (item: string, entityConfig: any) => {
    const videoElement = entityConfig.materialRef.value

    // ✅ 先删同 id 旧实体，避免二次开启时 duplicate id 抛错
    ...
    const onVideoReady = () => {
      ...
      if (ent?.polygon) {
        ent.polygon.material = videoElement // 换成视频材质
      }
      detach()
    }
    ...
    // ✅ 关键：视频已经在播（第二次开启全景）→ 直接换材质
    // 否则 waiting playing/loadeddata 永远等不到，贴图一直透明
    if (videoElement.readyState >= 2 && !videoElement.paused) {
      ...
      // 5 秒兜底：仍是透明占位（Color 实例）就强制换，防止一直黑屏
      timer = setTimeout(() => {
        const ent = viewer.entities.getById(item)
        if (ent?.polygon && ent.polygon.material instanceof Cesium.Color) {
          onVideoReady()
        }
      }, 5000)
    }

    // ✅ 启动取流，并把 stop 句柄登记起来，供关闭全景时真正断流
    videoStopHandles[item] = rtcVideo(videoElement, entityConfig.streamUrl)
  }
...
const removeAllVideos = (videoEntities: any[]) => {
  ...
  videoEntities.forEach((entityConfig) => {
    viewer.entities.removeById(entityConfig.id)
    // ✅ 真正断开 WebRTC、停轨道（旧代码 videoMaterial.video 不存在，是静默失败）
    videoStopHandles[entityConfig.id]?.()
    delete videoStopHandles[entityConfig.id]
  })
}
...
const yichushipin = () => {
  ...
  // ✅ 兜底：停掉所有残留流（覆盖视口切换等配置对不上的场景）
  Object.keys(videoStopHandles).forEach((id) => {
    videoStopHandles[id]?.()
    delete videoStopHandles[id]
  })
  console.log("所有视频实体及视频流已停止")
}

const setEntityVisibility = function (id: string, _visible: boolean) {
  const allEntities = QuanJing.getEntityConfigs()
  allEntities.forEach((item) => {
    if (item.id !== id) {
      viewer.entities.removeById(item.id)
      videoStopHandles[item.id]?.() // ✅ 移除实体的同时停流
      delete videoStopHandles[item.id]
    }
  })
}
...
const rtcVideo = (vdo: HTMLVideoElement, url: string) => {
  let webrtc: RTCPeerConnection | null = null
  let webrtcSendChannel: RTCDataChannel | null = null
  let webrtcSendChannelInterval: any = null
  let stopped = false

  const onLoaded = () => vdo.play().catch(() => { })
  vdo.addEventListener("loadeddata", onLoaded)
  ...
  // ✅ 对外暴露的停止方法：真正断流 + 停解码 + 清轨道
  const stop = () => {
    if (stopped) { return }
    stopped = true
    vdo.removeEventListener("loadeddata", onLoaded)
    cleanup()
    if (vdo.srcObject) {
      (vdo.srcObject as MediaStream).getTracks().forEach(t => t.stop())
      vdo.srcObject = null
    }
    try { vdo.pause() }
    catch { /* ignore */ }
  ...
  function startPlay() {
    if (stopped) { return } // ✅ 已停止就不允许重连
    cleanup() // 重连前先清旧连接
    webrtc = new RTCPeerConnection({ sdpSemantics: "unified-plan" })
    ...
    webrtcSendChannel.onclose = () => {
      ...
      startPlay() // stopped 时 startPlay 内部直接 return
    }
    webrtcSendChannel.onopen = () => {
      webrtcSendChannel!.send("ping")
      // ✅ 注意：这里必须是赋值（无 let），否则 TDZ 报错
  ...
  async function handleNegotiationNeeded() {
    ...
    $.post(url, { data: btoa(webrtc!.localDescription!.sdp) }, (data: string) => {
      try {
