...
import type { HotspotEntity } from "@/type/vMap"
...
// ===== 新增：动态区域显示/隐藏控制 =====
const showDynamicAreas = ref(false)
...
const convertAndAddOneHour = (inputStr: any) => {
  ...
  if (Number.isNaN(baseDate.getTime())) {
    throw new TypeError("Invalid input date string")
  }
...
const isimagelist1 = ref(false)

let SPfx = ref("")
const SPkzq = ref(false)

const OpenModel1 = function (id) {
  SPfx.value = id
  SPkzq.value = true
  const idToArrayMap = {
    q1: [
  ...
  const specialHandlers = {
    "q2,q1": () => {
      ...
      // const _dataArr = hasVideo ? [...idToArrayMap.q1, ...idToArrayMap.q2] : [];
...
<template>
  <div class="contont">
    ...
    <main v-else>
      ...
      <div v-if="showSmartDisplay" class="smart-display-panel">
        <div class="smart-display-grid">
          <div v-for="(item, i) in smartDisplayItems" :key="item.label" :ref="(el) => setSmartItemRef(el, i)"
               class="smart-display-item" :class="{ active: activeSmartItem === item.label }"
          ...
          >
            ...
            <span v-if="activeSmartItem === item.label && isCruiseItem(item.value)" class="cruise-stop-badge"
                  :class="{ paused: tingzhifeixing }" :title="tingzhifeixing ? '继续巡航' : '暂停巡航'"
                  @click.stop="handleCruiseToggle"
            >
      ...
      <!-- 底部ABC馆按钮容器 -->
      <div v-show="!showSmartDisplay" class="abc-buttons-container">
      ...
      <div class="center">
        ...
        <div class="map">
          ...
          <!-- 地图容器 -->
          <div class="chart">
            <vMap ref="vMapRef" @point-name="handleCruisePointChange" @play-video-fusion="playRTCVideoStream"
          ...
          <!-- 视频弹窗 -->
          <div v-if="isShow.isShowVideo" ref="videoRef" class="video-container"
          ...
          <div v-show="showChainMsgPopup" class="chain-msg-popup">
            ...
            <div class="popup-search-bar">
              <div class="search-row">
                <input v-model="searchAlarmParams.cameraName" type="text" placeholder="相机名称"
                       class="search-input name-input"
                />
                <input v-model="searchAlarmParams.beginTime" type="datetime-local" class="search-input time-input" />
                <span style="color: #00c6ff">至</span>
                <input v-model="searchAlarmParams.endTime" type="datetime-local" class="search-input time-input" />
                <!-- <button class="search-btn" >
                  搜索
                </button> -->
                <el-button class="alarm-search-btn" type="primary" :icon="Search"
                           @click="handleAlarmSearch"
                ></el-button>
              </div>
            </div>
...
.smart-display-item {
  ...
  &.active {
    ...
    p {
      background: linear-gradient(90deg, rgba(0, 198, 255, 0.55), rgba(0, 150, 200, 0.55)); // 渐变底色替代纯色，更精致
      color: #ffffff;
