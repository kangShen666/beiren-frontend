    ...
    success: function () {
      setTimeout(async function () {
        setTimeout(function () {
          ...
          setTimeout(function () {
            if (show) {
              clickStartRealPlay(ip, index,data.iChannelID);
              console.log('登录成功');
              // clickGoPreset(data.port)
            }
          }, 1000);
        }, 1000);
...
export function getChannelInfo() {
  ...
  WebVideoCtrl.I_GetAnalogChannelInfo(szDeviceIdentify, {
    success: function (xmlDoc: Document) {
      ...
      // console.log(channels.value,"111111111");
...
// 开始预览
export const clickStartRealPlay = (ip?: string, port?: number,iChannelID?:number) => {
