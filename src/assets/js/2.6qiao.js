...
class ModelColQiao {
    ...
    update(data, skip = 30) {
        ...
        this.eleIds.forEach(id => {
            ...
            // 条件：1.超过3秒未更新 2.本次更新数据中无该ID
            const isTimeout = currentTime - lastUpdateTime > 3000;
        ...
        needDeleteIds.forEach(id => {
            ...
            console.log(`ID为${id}的模型因3秒未更新已移除`);
        });
    ...
    removeAll() {
        ...
        for (let i = this.eles.length - 1; i >= 0; i--) {
            ...
            if (el) {
                ...
                // 调用实例自身的remove方法
                if (typeof el.removeAll === 'function') {
                    el.removeAll();
                }
                // 从数组删除
...
class Model {
    ...
    // 修复：统一方法名，原方法名removeAll改为remove（匹配调用处）
    removeAll() {
        if (this.animateEntity) {
            // 强制移除（忽略Cesium内部状态检查）
