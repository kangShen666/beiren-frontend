...
class ModelCol {
    constructor(viewer) {
        ...
        // 新增：位置更新距离阈值（米）
        this.positionUpdateThreshold = 2;
    }

    // 新增工具方法：计算两个经纬度坐标之间的地面距离（米）
    calculateGroundDistance(lon1, lat1, lon2, lat2) {
        if (!lon1 || !lat1 || !lon2 || !lat2) return 0;
        
        // 将经纬度转换为Cartesian3坐标
        const pos1 = Cesium.Cartesian3.fromDegrees(parseFloat(lon1), parseFloat(lat1), 0);
        const pos2 = Cesium.Cartesian3.fromDegrees(parseFloat(lon2), parseFloat(lat2), 0);
    ...
    update(data, skip = 30) {
        ...
        persons.forEach(d => {
            ...
            if (this.eleIds.includes(id)) {
                ...
                if (el) {
                    // 核心修改：计算当前位置与新位置的距离
                    const currentLon = el.currentLon || el.position?.longitude || null;
                    const currentLat = el.currentLat || el.position?.latitude || null;
                    
                    // 如果能获取到当前坐标，计算距离
                    if (currentLon && currentLat) {
                        const distance = this.calculateGroundDistance(currentLon, currentLat, d.x, d.y);
                        // 距离小于阈值，不更新位置
                        if (distance < this.positionUpdateThreshold) {
                            // console.log(`模型${id}位置变化${distance.toFixed(2)}米，小于${this.positionUpdateThreshold}米，不更新`);
                            return;
                        }
                    }
            ...
            // 新模型：创建实例
                ...
                try {
                    ...
                    // 记录初始坐标
                    newModel.currentLon = d.x;
                    newModel.currentLat = d.y;
                    // 关键修复5：调整实体有效性判断逻辑
...
class Model {
    constructor(id, viewer, type, position, text, direction, cameraName) {
        ...
        this.currentLon = null;        // 新增：记录当前经度
        this.currentLat = null;        // 新增：记录当前纬度
